...
from app.db.models import Comment, Issue, IssuePriority, IssueStatus, Project, User
...
def test_createcomment_success(db_session):
    ...
    toCreate = CommentCreate(issue_id=issueDB.public_id, body="Comment 1")
...
def test_createcomment_issuenotfound(db_session):
    ...
    password = "AAAAAAA"  # matches hashed
    token = get_test_token(c, login, password)
    toCreate = CommentCreate(issue_id="Invalid", body="Comment 1")
    r = c.post(
        "/comment/create",
        headers={"Authorization": f"Bearer {token.access_token}"},
        json=toCreate.model_dump(),
...
def test_readcomment_success(db_session):
    ...
    db_session.add(issueDB)
    commentDB = Comment(id=None, issue=issueDB, body="Comment 1", author=userDB)
...
def test_editcomment_success(db_session):
    ...
    toEdit = CommentEditIn(
        issue_id=commentDB.issue.public_id, body="Comment edited by author"
    )
...
def test_editcomment_usernotauthor(db_session):
    ...
    db_session.add(issueDB)
    commentDB = Comment(id=None, issue=issueDB, body="Comment 1", author=anotherUserDB)
    db_session.add(commentDB)
    db_session.commit()
    login = userDB.username
    password = "AAAAAAA"  # matches hashed
