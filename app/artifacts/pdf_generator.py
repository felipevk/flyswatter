...
def monthly_report_pdf(report: MonthlyReport, filename: str):
    ...
    pdf.set_fill_color(
        **asdict(pdfPallete[0])
    )  # asdict returns a dictionary, ** unpacks the dict into named args
    ...
    if not report.projects:
        ...
        for project in report.projects:
            ...
            pdf.cell(
                10, 10, f"{project.created_issues_month}", ln=True, fill=True, align="C"
            )
            pdf.cell(60, 10, "Closed Issues this month", fill=True)
            pdf.cell(
                10, 10, f"{project.closed_issues_month}", ln=True, fill=True, align="C"
            )
